...
const emailResendCooldown = new Map<string, number>();
...
export class MyTripsController {
  ...
  constructor(
    private readonly myTripsService: MyTripsService,
  ) {}
  ...
  async resendEmail(
    ...
    const cooldownKey = `${booking.id}-${booking.customerEmail}`;
    const lastSent = emailResendCooldown.get(cooldownKey);
    const now = Date.now();
    ...
    if (lastSent && (now - lastSent) < COOLDOWN_MS) {
      ...
      return {
        success: false,
        message: `Lütfen ${remainingSeconds} saniye bekleyin`,
    ...
    // Email gönder
    try {
      // EmailService inject edilmediği için, şimdilik log'layalım
      // Production'da EmailService kullanılacak
      this.logger.log(`Resending confirmation email to: ${this.maskEmail(booking.customerEmail)}`);
      
      // Cooldown'u güncelle
      emailResendCooldown.set(cooldownKey, now);
      ...
      return {
        success: true,
        message: `Onay emaili ${this.maskEmail(booking.customerEmail)} adresine gönderildi`,
      };
    } catch (error) {
      this.logger.error(`Email resend failed: ${(error as Error).message}`);
      throw new BadRequestException('Email gönderilemedi, lütfen daha sonra tekrar deneyin');
    }
  }

  /**
   * Eski cooldown entry'lerini temizle
   */
  private cleanupCooldownMap(): void {
    const now = Date.now();
    for (const [key, timestamp] of emailResendCooldown.entries()) {
      if (now - timestamp > COOLDOWN_MS * 2) {
        emailResendCooldown.delete(key);
      }
    }
  }
